...
#include "qedi_compat.h"
#include "nvm_iscsi_cfg.h"

#define QEDI_ISCSI_INTERFACE_VERSION	5906
#define QEDI_MODULE_NAME		"qedi"
#define QEDI_DESCR			"QLogic iSCSI Offload Driver"
...
#define MAX_ISCSI_LUNS			0xffff

#define QEDI_MAX_BD_LEN		0xffff
#define QEDI_BD_SPLIT_SZ	0x1000
#define QEDI_PAGE_SIZE		4096 /* 4K page */
...
struct dbg_tsk_info {
	...
	s16	id_failed;
	s16	id_c_failed;
	bool	cmd_node_active;
	u32	cid;
	u32     itt;
	struct	list_head *io_cmd_node;
...
#define QEDI_IO_TRACE_RSP              1
	...
	u8 op;		/* SCSI CDB */
	u8 lba[4];
	unsigned bufflen;	/* SCSI buffer length */
	unsigned sg_count;	/* Number of SG elements */
...
#define QEDI_GRCDUMP_SETUP	3
#define QEDI_GRCDUMP_CAPTURE	4
...
#define QEDI_INVALID_TASK_ID	(MAX_ISCSI_TASK_ENTRIES + 1)
	...
	u8 *grcdump;
	u32 grcdump_size;
	/* TBD: Need to protect this variable*/
	u16 ll2_mtu;
	...
	struct delayed_work recovery_process_work;

	spinlock_t task_idx_lock;	/* To protect gbl context */
	s32 last_tidx_alloc;
	s32 last_tidx_clear;
	struct dbg_tsk_info tin[MAX_ISCSI_TASK_ENTRIES];
 ...
 *
 * void *ptr;
 * ptr = qedf_get_task_mem(&qedi->tasks, 128);
 */
...
#ifndef __vlan_insert_tag
#define __vlan_insert_tag	__vlan_put_tag
#endif
#endif
...
#ifdef ERROR_INJECT
extern void qedi_get_protocol_tlv_data(void *dev, void *data);
extern void qedi_get_generic_tlv_data (void *dev,
	struct qed_generic_tlvs *data);
extern void qedi_schedule_hw_err_handler(void *dev,
