...
/*
 * We need these defines so the right parts of qed will be compiled in
 * correctly
 */
#define CONFIG_QED_L2
/* This is already defined in some distro configs */
#ifndef CONFIG_QED_SRIOV
#define CONFIG_QED_SRIOV
#endif
#ifndef CONFIG_QED_ROCE
#define CONFIG_QED_ROCE
#endif
#ifndef CONFIG_QED_FCOE
#define CONFIG_QED_FCOE
#endif
#ifndef CONFIG_QED_ISCSI
#define CONFIG_QED_ISCSI
#endif
#ifndef CONFIG_QED_LL2
#define CONFIG_QED_LL2
#endif
#define _DEFINE_IFLA_VF_LINKSTATE
...
/* The version of qed we're currently supporting */
#define QEDF_FCOE_INTERFACE_VERSION		5906
...
#define MAX_DATA_FIELD_SIZE	2048
...
#define	QEDFC_CMD_ST_ELS_ACTIVE		12
	...
	struct e4_fcoe_task_params *task_params;
...
#define QEDF_DCBX_DONE		1
	atomic_t dcbx;
	enum qedf_feature dcbx_session;
	enum qedf_validity vprio;
	enum qedf_feature pfc;
...
#ifdef ERROR_INJECT
	/* Used to store various error injection events */
	int drop_cmd; /* Debug to drop SCSI completion */
	int drop_els; /* Debug to drop middle path ELS completion */
	int drop_abts; /* Debug to drop ABTS completion */
	int drop_cleanup; /* Debug to drop a cleanup completion */
	int drop_abts_queue; /* Debug to drop queuing an ABTS request */
	int no_abts_tmo; /* Do not complete timeout of abts */
	int error_detect; /* Simulate error detect CQE */
	int fcp_prot_error;
	int underrun_error; /* Simulate a firmware detected underrun */
	int abts_acc; /* Simulate ABTS accept status */
	int drop_tmf; /* Debug to drop task management completion */
	int fail_post_io_req; /* Fail qedf_post_io_req */
	int dbg_null_device; /* Simulate sc_cmd->device being NULL */ 
#endif
 ...
 * Externs
 */
/*(QEDF_LOG_NPIV | QEDF_LOG_SESS | QEDF_LOG_LPORT | QEDF_LOG_ELS | QEDF_LOG_MQ | QEDF_LOG_IO | QEDF_LOG_UNSOL | QEDF_LOG_SCSI_TM | QEDF_LOG_MP_REQ | QEDF_LOG_EVT | QEDF_LOG_CONN | QEDF_LOG_DISC | QEDF_LOG_INFO)
 */
...
extern int qedf_queuecommand(struct Scsi_Host *host,
	struct scsi_cmnd *sc_cmd);
#else
extern int qedf_queuecommand(struct scsi_cmnd *sc_cmd,
...
#else
extern void qedf_fip_recv(struct qedf_ctx *qedf, struct sk_buff *skb, u16 vlan);
#endif
...
extern void qedf_initiate_cleanup(struct qedf_ioreq *io_req,
	bool return_scsi_cmd_on_abts);
...
extern void qedf_scsi_done(struct qedf_ctx *qedf, struct qedf_ioreq *io_req,
	int result);
extern void qedf_set_vlan_id(struct qedf_ctx *qedf, uint32_t vlan_id);
...
extern bool qedf_wait_for_upload(struct qedf_ctx *qedf);
...
extern void qedf_schedule_recovery_handler(void *dev);
extern void qedf_recovery_handler(struct work_struct *work);
...
extern void qedf_hw_attr_update(void *dev, enum qed_hw_info_change attr);
extern void qedf_get_generic_tlv_data (void *dev,
	struct qed_generic_tlvs *data);
extern void qedf_wq_grcdump(struct work_struct *work);
extern void qedf_stag_change_work(struct work_struct *work);
extern void qedf_ctx_soft_reset(struct fc_lport *lport);
extern int qedf_get_fcoe_capabilities(void *dev, struct qed_fcoe_caps *p_caps);
...
#define MIN_NUM_CPUS_MSIX(x)	min((u32)x->dev_info.num_cqs, num_online_cpus())
...
/*
 * Set this to 1 to dump the entries of the structs we're passing the
 * HSI functions.
 */
#define QEDF_DUMP_TASK_FIELD		0
