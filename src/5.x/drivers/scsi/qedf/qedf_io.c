...
static void qedf_cmd_timeout(struct work_struct *work)
{
	...
	case QEDF_ABTS:
		if (qedf == NULL) {
			QEDF_INFO(NULL, QEDF_LOG_IO, "qedf is NULL for ABTS xid=0x%x.\n",
			    io_req->xid);
			return;
		}
		...
		if (qedf->no_abts_tmo) {
			qedf->no_abts_tmo--;
			QEDF_ERR(&qedf->dbg_ctx, "Short circuit ABTS timeout for xid=0x%x.\n",
			    io_req->xid);
			return;
		}
#endif
		 ...
		 * cleaned up the task.
		 */
		kref_put(&io_req->refcount, qedf_release_cmd);	/* ID: 002 */
	...
	case QEDF_SEQ_CLEANUP:
		...
		break;
	default:
		QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_IO, "Hit default case, "
		    "xid=0x%x.\n", io_req->xid);
		break;
	}
}
...
struct qedf_cmd_mgr *qedf_cmd_mgr_alloc(struct qedf_ctx *qedf)
{
	...
	for (i = 0; i < num_ios; i++) {
		...
		io_req->task_params = kzalloc(sizeof(struct e4_fcoe_task_params),
		    GFP_KERNEL);
		if (!io_req->task_params) {
			QEDF_ERR(&(qedf->dbg_ctx), "Failed to allocate "
			    "task_params for xid=0x%x.\n", i);
			goto mem_err;
		}
...
struct qedf_ioreq *qedf_alloc_cmd(struct qedf_rport *fcport, u8 cmd_type)
{
	...
	if (test_bit(QEDF_CMD_DIRTY, &io_req->flags))
		QEDF_ERR(&(qedf->dbg_ctx), "io_req found to be dirty "
			  "ox_id = 0x%x.\n", io_req->xid);
...
void qedf_release_cmd(struct kref *ref)
{
	...
	if (io_req->cmd_type == QEDF_SCSI_CMD) {
		QEDF_WARN(&(fcport->qedf->dbg_ctx),
		    "Cmd released called without scsi_done called, io_req %px "
		    "xid=0x%x.\n", io_req, io_req->xid);
	...
	struct fcoe_wqe *sqe)
{
	...
	if (io_req->tm_flags) {
		cq_idx = smp_processor_id() % qedf->num_queues;
		goto skip_blk_mq_tag;
	}
	...
	QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_DISC,
		  "uniq_tag=0x%x hwq=0x%x cq_idx=0x%x.\n", uniq_tag, hwq, cq_idx);
#endif
skip_blk_mq_tag:
	...
	memset(io_req->task_params, 0, sizeof(struct e4_fcoe_task_params));
	...
	/* Setup the fields for e4_fcoe_task_params */
...
#if QEDF_DUMP_TASK_FIELD == 1 
	pr_crit("%s(): tx_io_size=%d rx_io_size=%d.\n", __func__, tx_io_size, rx_io_size);
	pr_crit("%s(): task_ctx=%px.\n", __func__, io_req->task_params->context);
	pr_crit("%s(): sqe=%px.\n", __func__, io_req->task_params->sqe);
	pr_crit("%s(): task_type=%d.\n", __func__, io_req->task_params->task_type);
	pr_crit("%s(): tx_io_size=%d rx_io_size=%d.\n", __func__, io_req->task_params->tx_io_size, io_req->task_params->rx_io_size);
	pr_crit("%s(): conn_cid=%04x.\n", __func__, io_req->task_params->conn_cid);
	pr_crit("%s(): itid=0x%x.\n", __func__, io_req->xid);
	pr_crit("%s(): cq_rss_number=%d.\n", __func__, io_req->task_params->cq_rss_number);
	pr_crit("%s(): is_tape_device=%d.\n", __func__, io_req->task_params->is_tape_device);
	for (i = 0; i < bd_count; i++) {
		pr_crit("%s(): len=%d hi=0x%x lo=0x%x.\n", __func__, bd_tbl->bd_tbl[i].sge_len, bd_tbl->bd_tbl[i].sge_addr.hi, bd_tbl->bd_tbl[i].sge_addr.lo);
	}
	pr_crit("%s(): sgl=%px.\n", __func__, io_req->sgl_task_params->sgl);
	pr_crit("%s(): sgl_phys_add hi=0x%x lo=0x%x.\n", __func__, io_req->sgl_task_params->sgl_phys_addr.hi, io_req->sgl_task_params->sgl_phys_addr.lo);
	pr_crit("%s(): num_sges=%d.\n", __func__, io_req->sgl_task_params->num_sges);
	pr_crit("%s(): total_buffer_size=%d.\n", __func__, io_req->sgl_task_params->total_buffer_size);
	pr_crit("%s(): small_mid_sge=%d.\n", __func__, io_req->sgl_task_params->small_mid_sge);
	pr_crit("%s(): sense_data_buffer_phys_addr hi=0x%x lo=0x%x.\n", __func__, sense_data_buffer_phys_addr.hi, sense_data_buffer_phys_addr.lo);
	pr_crit("%s(): task_retry_identifier=0x%x.\n", __func__, io_req->task_retry_identifier);
	print_hex_dump(KERN_WARNING, "fcp_cmnd ", DUMP_PREFIX_OFFSET, 16, 1,
	    fcp_cmnd, 32, false);
#endif

	init_initiator_rw_fcoe_task(io_req->task_params,
	    io_req->sgl_task_params, sense_data_buffer_phys_addr,
	...
	struct fcoe_task_context *task_ctx, struct fcoe_wqe *sqe)
{
	...
	/* DMA memory info for request buffer */
	struct scsi_sgl_task_params tx_sgl_task_params;
	/* DMA memory info for response buffer */
	...
	io_req->task_params->rx_io_size = QEDF_PAGE_SIZE;
	io_req->task_params->conn_cid = fcport->fw_cid;
	io_req->task_params->itid = io_req->xid;
	...
	/* Set PAGE_SIZE for now since sg element is that size ??? */
	rx_sgl_task_params.total_buffer_size = QEDF_PAGE_SIZE;
	rx_sgl_task_params.small_mid_sge = 0;

#if QEDF_DUMP_TASK_FIELD == 1
	pr_crit("%s(): task_params->context=0x%px.\n", __func__, io_req->task_params->context);
	pr_crit("%s(): task_params->sqe=0x%px.\n", __func__, io_req->task_params->sqe);
	pr_crit("%s(): task_params->task_type=%d.\n", __func__, io_req->task_params->task_type);
	pr_crit("%s(): task_params->tx_io_size=%d.\n", __func__, io_req->task_params->tx_io_size);
	pr_crit("%s(): task_params->rx_io_size=%d.\n", __func__, io_req->task_params->rx_io_size);
	pr_crit("%s(): task_params->conn_cid=%04x.\n", __func__, io_req->task_params->conn_cid);
	pr_crit("%s(): task_params->itid=0x%x.\n", __func__, io_req->task_params->itid);
	pr_crit("%s(): task_params->cq_rss_number=%d.\n", __func__, io_req->task_params->cq_rss_number);
	pr_crit("%s(): task_params->is_tape_device=%d.\n", __func__, io_req->task_params->is_tape_device);
	pr_crit("%s(): task_fc_hdr.parameter=0x%x.\n", __func__, task_fc_hdr.parameter);
	pr_crit("%s(): task_fc_hdr.r_ctl=0x%x.\n", __func__, task_fc_hdr.r_ctl);
	pr_crit("%s(): task_fc_hdr.type=0x%x.\n", __func__, task_fc_hdr.type);
	pr_crit("%s(): task_fc_hdr.cs_ctl=0x%x.\n", __func__, task_fc_hdr.cs_ctl);
	pr_crit("%s(): task_fc_hdr.df_ctl=0x%x.\n", __func__, task_fc_hdr.df_ctl);
	pr_crit("%s(): task_fc_hdr.rx_id=0x%x.\n", __func__, task_fc_hdr.rx_id);
	pr_crit("%s(): task_fc_hdr.ox_id=0x%x.\n", __func__, task_fc_hdr.ox_id);
	pr_crit("%s(): tx_sgl_task_params.sgl=0x%px.\n", __func__, tx_sgl_task_params.sgl);
	pr_crit("%s(): tx_sgl_task_params.sgl_phys_addr.lo=0x%08x", __func__, tx_sgl_task_params.sgl_phys_addr.lo);
	pr_crit("%s(): tx_sgl_task_params.sgl_phys_addr.hi=0x%08x", __func__, tx_sgl_task_params.sgl_phys_addr.hi);
	...
	pr_crit("%s(): rx_sgl_task_params.sgl=0x%px.\n", __func__, rx_sgl_task_params.sgl);
	pr_crit("%s(): rx_sgl_task_params.sgl_phys_addr.lo=0x%08x", __func__, rx_sgl_task_params.sgl_phys_addr.lo);
	pr_crit("%s(): rx_sgl_task_params.sgl_phys_addr.hi=0x%08x", __func__, rx_sgl_task_params.sgl_phys_addr.hi);
	...
	init_initiator_midpath_unsolicited_fcoe_task(io_req->task_params,
	    &task_fc_hdr, &tx_sgl_task_params, &rx_sgl_task_params, 0);

#if QEDF_DUMP_TASK_FIELD == 1
	pr_crit("%s(): sqe->task_id=%04x sqe->flags=%04x.\n", __func__, sqe->task_id, sqe->flags);
#endif
}
...
#ifdef ERROR_INJECT
	if (qedf->fail_post_io_req) {
		qedf->fail_post_io_req--;
		QEDF_ERR(&(qedf->dbg_ctx), "Fail posting of io_req xid=0x%x.\n",
		    io_req->xid);
		/* Release cmd will release io_req, but sc_cmd is assigned */
		io_req->sc_cmd = NULL;
		kref_put(&io_req->refcount, qedf_release_cmd);	/* ID: 001 */
...
#else
qedf_queuecommand(struct scsi_cmnd *sc_cmd, void (*done)(struct scsi_cmnd *))
#endif
{
#if (LINUX_VERSION_CODE < 0x20626)
	struct Scsi_Host *host = sc_cmd->device->host;
	...
	if (fcport->retry_delay_timestamp) {
		/* Take fcport->rport_lock for reseting the delay_timestamp */
				 ...
				 struct fcoe_cqe_rsp_info *fcp_rsp)
{
	...
	uint32_t fcp_sns_len = 0;
	uint32_t fcp_rsp_len = 0;
	...
	struct qedf_ioreq *io_req)
{
	...
	if (!test_bit(QEDF_CMD_OUTSTANDING, &io_req->flags) ||
		...
		QEDF_ERR(&(qedf->dbg_ctx), "io_req xid=0x%x sc_cmd=%px already "
			  "in cleanup or abort processing or already "
			  "completed, flags = 0x%lx.\n",
			  io_req->xid, io_req->sc_cmd, io_req->flags);
		return;
	}
...
#ifdef ERROR_INJECT
	if (qedf->fcp_prot_error) {
		qedf->fcp_prot_error--;
		io_req->fcp_rsp_code = 1;
		io_req->fcp_rsp_len = 4;
	}
#endif
...
#ifdef ERROR_INJECT
	if (qedf->underrun_error) {
		qedf->underrun_error--;
		cqe->cqe_info.rsp_info.fw_error_flags |=
		    FCOE_CQE_RSP_INFO_FW_UNDERRUN_MASK;
	}
#endif
	...
	if (fw_residual_flag) {
		QEDF_ERR(&(qedf->dbg_ctx),
		    ...
		    "fcp_resid=0x%d fw_residual=0x%x lba=%02x%02x%02x%02x.\n",
		    io_req->xid, fcp_rsp->rsp_flags.flags, io_req->fcp_resid,
				...
				if (qedf_retry_delay) {
					if ((scope == 1 || scope == 2) &&
						...
						if (qualifier > QEDF_RETRY_DELAY_MAX) {
							...
							QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_IO,
							"Scope = %d and qualifier = %d and driver enforced delay is %d\n",
							 scope, (fcp_rsp->retry_delay_timer & 0x3FFF), qualifier);
						} else {
							QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_IO,
							"Scope = %d and qualifier = %d", scope, qualifier);
						}
						...
						QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_IO,
						"combination of scope = %d and qualifier = %d"
						"is not handled in qedf.\n", scope, qualifier);
					}
				}
				/* Record stats */
	...
	int result)
{
	...
	if (!virt_addr_valid(sc_cmd->device)) {
		QEDF_ERR(&qedf->dbg_ctx, "Device pointer for sc_cmd %px is bad.\n",
		    sc_cmd);
		goto bad_scsi_ptr;
	}

	if (!sc_cmd->sense_buffer) {
		QEDF_ERR(&qedf->dbg_ctx, "sc_cmd->sense_buffer for sc_cmd %px is NULL.\n",
		    sc_cmd);
		goto bad_scsi_ptr;
	}

	if (!virt_addr_valid(sc_cmd->sense_buffer)) {
		QEDF_ERR(&qedf->dbg_ctx, "sc_cmd->sense_buffer for sc_cmd %px is bad.\n",
		    sc_cmd);
		goto bad_scsi_ptr;
	}
...
#endif
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,2,0)
	QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_IO, "%d:0:%d:%u: Completing "
#else
...
bad_scsi_ptr:
	/*
	 * Clear the io_req->sc_cmd backpointer so we don't try to proces
	 * this again
	 */
	io_req->sc_cmd = NULL;
	kref_put(&io_req->refcount, qedf_release_cmd);	/* ID: 001 */
	...
	struct qedf_ioreq *io_req)
{
	...
	QEDF_ERR(&(io_req->fcport->qedf->dbg_ctx),
	    "Error detection CQE, xid=0x%x err_warn_bitmap=%08x:%08x tx_buff_off=%08x rx_buff_off=%08x, rx_id=%04x\n",
	    io_req->xid, le32_to_cpu(cqe->cqe_info.err_info.err_warn_bitmap_hi),
	    le32_to_cpu(cqe->cqe_info.err_info.err_warn_bitmap_lo),
	    le32_to_cpu(cqe->cqe_info.err_info.tx_buf_off),
	    le32_to_cpu(cqe->cqe_info.err_info.rx_buf_off),
	    le32_to_cpu(cqe->cqe_info.err_info.rx_id));
...
void qedf_flush_active_ios(struct qedf_rport *fcport, int lun)
{
	...
	if (test_bit(QEDF_RPORT_UPLOADING_CONNECTION, &fcport->flags) && (lun == -1)) {
	...
	QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_IO, "Flush active i/o's num=0x%x "
	    "fcport=0x%px port_id=0x%06x scsi_id=%d.\n", atomic_read(&fcport->num_active_ios),
	    fcport, fcport->rdata->ids.port_id, fcport->rport->scsi_target_id);
	...
	for (i = 0; i < FCOE_PARAMS_NUM_TASKS; i++) {
		 ...
		 * but RRQ is still pending. Handle this case in 12.40.
...
//			(io_req != QEDF_ABTS)) {
		if (!test_bit(QEDF_CMD_OUTSTANDING, &io_req->flags)) {
			...
			/* If RRQ work has been queue, try to cancel it and free the io_req */
			if (atomic_read(&io_req->state) == QEDFC_CMD_ST_RRQ_WAIT) {
				if (cancel_delayed_work_sync(&io_req->rrq_work)) {
					QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_IO,
					    "Putting reference for pending RRQ work xid=0x%x.\n",
					    io_req->xid);
					kref_put(&io_req->refcount, qedf_release_cmd);	/* ID: 003 */
				}
			}
			continue;
		}
		...
		if (io_req->cmd_type == QEDF_ABTS) {
			rc = kref_get_unless_zero(&io_req->refcount);	/* ID: 004 */
			if (!rc) {
				QEDF_ERR(&(qedf->dbg_ctx),
			...
			/* TODO: GSS: This is currently relevant in the window when we have
			 * queued up rrq_work but have not yet called scsi_done (when the
			 * OUTSTANDING bit gets cleared). Once we move to io_req->state based
			 * handling, this can be removed from here.
			 */
			if (cancel_delayed_work_sync(&io_req->rrq_work)) {
				QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_IO,
		...
		if (!io_req->sc_cmd)
			continue;
#ifdef ERROR_INJECT
		if (qedf->dbg_null_device) {
			qedf->dbg_null_device--;
			QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_IO,
			    "Simulating null sc_cmd->device pointer xid=0x%x sc_cmd=%px.\n",
			    io_req->xid, io_req->sc_cmd);
			io_req->sc_cmd = NULL;
			qedf_initiate_cleanup(io_req, false);
			/* Put reference for non-existant scsi_cmnd */
			kref_put(&io_req->refcount, qedf_release_cmd);
			continue;
		}
#endif
		...
		if (fcport->rport->scsi_target_id != io_req->sc_cmd->device->id) {
			QEDF_ERR(&qedf->dbg_ctx,
			    "Crashing !!! (scsi_targe_id)0x%x != 0x%x(device_id), fcport=%px io_req=%px "
			    "xid=0x%x.\n", fcport->rport->scsi_target_id, io_req->sc_cmd->device->id,
			    fcport, io_req, io_req->xid);
			BUG_ON(1);
		}
		...
		/*
		 * During the MFW recovery process, cleanup commands will not
		 * be responded to so issuing cleanup commands will just bog
		 * down the upload process.  If we are in recovery simply
		 * return the commands back to the SCSI layer with a retryable
		 * error.  In all other contexts issue the cleanup request so
		 * the firmware has no stale context.
		 */
...
free_cmd:
	...
	wait_cnt = 60;
	QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_IO, "Flushed 0x%x I/Os, active=0x%x.\n",
	...
	if (test_bit(QEDF_RPORT_UPLOADING_CONNECTION, &fcport->flags) && (lun == -1)) {
		while (atomic_read(&fcport->num_active_ios)) {
			QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_IO, "Flushed 0x%x I/Os, active=0x%x cnt=%d.\n",
			    flush_cnt, atomic_read(&fcport->num_active_ios), wait_cnt);
			if (wait_cnt == 0) {
				QEDF_ERR(&qedf->dbg_ctx, "Flushed %d I/Os, active=%d.\n",
				flush_cnt, atomic_read(&fcport->num_active_ios));
				for (i = 0; i < FCOE_PARAMS_NUM_TASKS; i++) {
					io_req = &cmd_mgr->cmds[i];
					if (io_req->fcport != NULL &&
						...
						refcount = atomic_read(&io_req->refcount.refcount);
#endif
						set_bit(QEDF_CMD_DIRTY, &io_req->flags);
						QEDF_ERR(&qedf->dbg_ctx,
							"Outstanding io_req =%px xid=0x%x "
							"flags=0x%lx, sc_cmd=%px refcount=%d "
							"cmd_type=%d.\n",
							io_req, io_req->xid,
							io_req->flags,
							io_req->sc_cmd, refcount,
							io_req->cmd_type);
					}
				}
				if (qedf_crash_on_err)
					BUG_ON(atomic_read(&fcport->num_active_ios));
				else
					WARN_ON(atomic_read(&fcport->num_active_ios));
				break;
			}
			msleep(500);
			wait_cnt--;
...
int qedf_initiate_abts(struct qedf_ioreq *io_req, bool return_scsi_cmd_on_abts)
{
	...
	qedf = fcport->qedf;
	rdata = fcport->rdata;
	/* TODO: GSS: This needs to be taken out after all callers
	 * get a reference before invoking this function
	 */
	...
	if (lport->vport)
		...
		base_lport = lport;
	phys_qedf = lport_priv(base_lport);

	if (phys_qedf->drop_abts_queue) {
		phys_qedf->drop_abts_queue--;
		rc = 1;
		goto out;
	}
#endif

	/* TODO: GSS: This needs to be removed after all
	 * the callers get a reference on io_req.
	 */
	kref_get(&io_req->refcount);	/* ID: 002 */
	...
	struct qedf_ioreq *io_req)
{
	...
	/* GSS: This was added at a point when we were scheduling abts_compl &
	 ...
	 * This check can be taken out if we do not hit this condition
	 * during the 12.40 qual cycle.
	 */
	if (fcport == NULL) {
		QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_IO,
	...
	if(!cancel_delayed_work(&io_req->timeout_work)) {
		/* TODO: GSS: This would cause a reference count issue */
		QEDF_ERR(&(qedf->dbg_ctx), "Wasn't able to cancel abts "
			"timeout work.\n");
	}

#ifdef ERROR_INJECT
	if (qedf->abts_acc) {
		/* Force accept status to trigger send RRQ */
		r_ctl = FC_RCTL_BA_ACC;
		qedf->abts_acc--;
	}
#endif
	switch (r_ctl) {
 ...
 * FAILURE.
 */
void qedf_initiate_cleanup(struct qedf_ioreq *io_req,
...
process_els:
	/* Ensure room on SQ */
	/* TODO we need to check if this condition need to be handled
 	 * after changing to return status to void.
 	 */
	if (!atomic_read(&fcport->free_sqes)) {
		QEDF_ERR(&(qedf->dbg_ctx), "No SQ entries available\n");
	...
	uint8_t tm_flags)
{
	 ...
	 */
	if (test_bit(QEDF_RPORT_UPLOADING_CONNECTION, &fcport->flags)) {
		QEDF_ERR(&qedf->dbg_ctx, "fcport is uploading, not executing flush.\n");
		goto no_flush;
	}
...
int qedf_initiate_tmf(struct scsi_cmnd *sc_cmd, u8 tm_flags)
{
	...
	struct fc_rport_priv *rdata = fcport->rdata;
        ...
        QEDF_ERR(NULL, "qedf_initiate_tmf tm_flags 0x%x sc_cmd %px "
            "op = 0x%02x target_id = 0x%x lun=%d\n", tm_flags, sc_cmd,
	    sc_cmd->cmd_len ? sc_cmd->cmnd[0] : 0xff, rport->scsi_target_id,
...
#else
		ref_cnt = atomic_read(&io_req->refcount.refcount);
#endif
		QEDF_ERR(NULL, "qedf_initiate_tmf orig io_req = %px xid = 0x%x "
	...
	struct fcoe_cqe *cqe)
{
	...
	QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_UNSOL,
	    ...
	    le32_to_cpu(cqe->cqe_info.unsolic_info.bd_info.opaque.fcoe_opaque.hi),
	    le32_to_cpu(cqe->cqe_info.unsolic_info.bd_info.opaque.fcoe_opaque.lo),
	    qedf->bdq_prod_idx, pktlen);

	bdq_idx = le32_to_cpu(cqe->cqe_info.unsolic_info.bd_info.opaque.fcoe_opaque.lo);
	...
	QEDF_WARN(&qedf->dbg_ctx,
		  "Processing Unsolicated frame, src=%06x dest=%06x r_ctl=%x type=%x cmd=%02x\n",
