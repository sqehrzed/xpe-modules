...
static pci_ers_result_t qedf_pci_error_detected(struct pci_dev *pdev, pci_channel_state_t state);
static pci_ers_result_t qedf_pci_slot_reset(struct pci_dev *pdev);
static void qedf_pci_resume(struct pci_dev *pdev);

/*
 * Driver module parameters.
 */
int qedf_enforce_lossless_eth;
module_param_named(enforce_lossless_eth, qedf_enforce_lossless_eth, int, 0644);
MODULE_PARM_DESC(enforce_lossless_eth,
	"Operate FCoE sessions only when transport remains Lossless (Relax = 0, Enforce = 1, default 0).");
...
static bool qedf_dcbx_no_wait = false;
...
MODULE_PARM_DESC(dp_level, " printk verbosity control passed to qed module  "
	"during probe (0-3: 0 more verbose, default 2).");

bool qedf_crash_on_err = false;
module_param_named(crash_on_err, qedf_crash_on_err, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(crash_on_err, " Crash the system when an unexpected event occours"
	"1 = Crash, 0 = Do not Crash (Default: 0).");

#ifdef NR_HW_QUEUES
static bool qedf_use_blk_mq = false;
module_param_named(use_blk_mq, qedf_use_blk_mq, bool, S_IRUGO);
MODULE_PARM_DESC(use_blk_mq, " Use of blk-mq (via scsi-mq) (default: off).");

static uint qedf_blk_mq_can_queue;
module_param_named(blk_mq_can_queue, qedf_blk_mq_can_queue, uint, S_IRUGO);
MODULE_PARM_DESC(blk_mq_can_queue, " Sets the queue depth of each hardware "
	"queue when blk-mq is enabled (default 0 which divides the queue "
	"depth evenly among all hardware queues).");
#endif

static bool qedf_enable_recovery = true;
module_param_named(enable_recovery, qedf_enable_recovery, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(enable_recovery, "Enable/disable recovery on driver/firmware interface level errors"
...
void qedf_set_vlan_id(struct qedf_ctx *qedf, uint32_t vlan_id)
{
	uint32_t vlan_id_tmp = 0;
	vlan_id_tmp = vlan_id  | (qedf->prio << VLAN_PRIO_SHIFT);
	qed_ops->update_fcoe_cvid(qedf->cdev, vlan_id_tmp);
	qedf->vlan_id = vlan_id_tmp;
	QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_DISC, "Setting vlan_id=0x%04x "
...
{
	bool fip_special = qed_ops->common->is_fip_special_mode(qedf->cdev);
	...
	while (qedf->fipvlan_retries) {
		...
		/*
		 * For all environments where fip_special is false only do x
		 * retries.
		 */
		if (!fip_special)
			qedf->fipvlan_retries--;
...
void qedf_dscp_pfc_enabled(struct qedf_ctx *qedf)
{
	uint32_t dscp_index;
	struct qed_hwfn *p_hwfn = &(qedf->cdev->hwfns[0]);

	if (!p_hwfn->p_dcbx_info->get.operational.valid) {
		QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_INFO,
		    "DCBX information not available.\n");
		return;
	}

	for (dscp_index = 0; dscp_index < QED_DCBX_DSCP_SIZE; dscp_index++) {
		if (p_hwfn->p_dcbx_info->get.dscp.enabled)
			QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_INFO,
			    "Dscp_index 0x%x Enabled.\n", dscp_index);
	}
}

void qedf_dscp_pfc_get(struct qedf_ctx *qedf)
{
	uint32_t dscp_index;
	int prio = -1;
	struct qed_hwfn *p_hwfn = &(qedf->cdev->hwfns[0]);
	...
	for (dscp_index = 0; dscp_index < QED_DCBX_DSCP_SIZE; dscp_index++) {
		if ((prio = p_hwfn->p_dcbx_info->get.dscp.dscp_pri_map[dscp_index]))
			QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_INFO,
			    "Dscp_index 0x%x pri = 0x%x.\n", dscp_index, prio);
	}
}

int qedf_dcbx_get_mode(struct qedf_ctx *qedf)
{
	struct qed_hwfn *p_hwfn = &(qedf->cdev->hwfns[0]);
...
void qedf_dcbx_get_pfc(struct qedf_ctx *qedf)
{
	uint32_t index;
	struct qed_hwfn *p_hwfn = &(qedf->cdev->hwfns[0]);
	...
	for (index = 0; index < QED_MAX_PFC_PRIORITIES; index++) {
		if (p_hwfn->p_dcbx_info->get.operational.params.pfc.prio[index])
			QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_INFO,
			    "For Index 0x%x priority is ENABLED.\n", index);
	}
}

void qedf_dcbx_get_pri_to_tc(struct qedf_ctx *qedf)
{
	uint32_t prio;
	uint8_t tc;
	struct qed_hwfn *p_hwfn = &(qedf->cdev->hwfns[0]);
	...
	for (prio = 0; prio < QED_MAX_PFC_PRIORITIES; prio++) {
		if ((tc = p_hwfn->p_dcbx_info->\
			get.operational.params.ets_pri_tc_tbl[prio]))
			QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_INFO,
			    "For Prio %d tc = %d.\n", prio, tc);
	}
}

void qedf_dcbx_get_tc_bw(struct qedf_ctx *qedf)
{
	uint32_t tc;
	uint8_t bw;
	struct qed_hwfn *p_hwfn = &(qedf->cdev->hwfns[0]);
	...
	for (tc = 0; tc < QED_MAX_PFC_PRIORITIES; tc++) {
		if ((bw = p_hwfn->p_dcbx_info->get.operational.params.ets_tc_bw_tbl[tc]))
			QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_INFO,
			    "TC %d BW = %d%%.\n", tc, bw);
	}
}

void qedf_dcbx_get_tc_tsa(struct qedf_ctx *qedf)
{
	uint32_t tc;
	uint8_t tsa;
	struct qed_hwfn *p_hwfn = &(qedf->cdev->hwfns[0]);
	...
	for (tc = 0; tc < QED_MAX_PFC_PRIORITIES; tc++) {
		if ((tsa = p_hwfn->p_dcbx_info->get.operational.params.ets_tc_tsa_tbl[tc]))
			QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_INFO,
			    "TC %d TSA = %d.\n", tc, tsa);
	}
}

static uint qedf_reset_fipvlan_retries(struct qedf_ctx *qedf)
{
	return qedf_fipvlan_retries;
}
...
static void qedf_handle_link_update(struct work_struct *work)
{
	...
	if (atomic_read(&qedf->link_state) == QEDF_LINK_UP) {
		 ...
		 * fip vlan request so set the vlan_id to either what the MFW
		 * has stored for UFP mode or the global fallback vlan
		 */
		if (test_bit(QED_MF_UFP_SPECIFIC, &qedf->cdev->mf_bits)) {
			QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_DISC,
			    "UFP mode is enabled.\n");
			fallback_vlan = qedf->cdev->hwfns[0].hw_info.ovlan;
		} else
			fallback_vlan = qedf_fallback_vlan;
 
		QEDF_WARN(&(qedf->dbg_ctx), "Did not receive FIP VLAN "
			   "response, falling back to default VLAN %d.\n",
			   fallback_vlan);
		qedf_set_vlan_id(qedf, fallback_vlan);
		...
		if (qedf_wait_for_upload(qedf) == false)
			QEDF_ERR(&qedf->dbg_ctx, "Cound not upload all sessions.\n");
		/* Reset the number of FIP VLAN retries */
		qedf->fipvlan_retries = qedf_reset_fipvlan_retries(qedf);
		/*
		 * Zero out data_src_addr so we'll update it with the new
		 * lport port_id
		 */
	...
	void *arg)
{
	...
	if (fc_frame_payload_op(fp) == ELS_LS_RJT)
		...
		/* Only send fabric name to MFW for physical port */
		if (!lport->vport) {
			flp = fc_frame_payload_get(fp, sizeof(*flp));
			/* Payload can be null so need to check */
			if (flp) {
				csp_flags = ntohs(flp->fl_csp.sp_features);
				/* Make sure this isn't vn2vn */
				if ((csp_flags & FC_SP_FT_FPORT) != 0) {
					fabric_name =
					    get_unaligned_be64(&flp->fl_wwnn);
					QEDF_INFO(&(qedf->dbg_ctx),
					    QEDF_LOG_DISC,
					    "fabric_name=0x%llx\n",
					    fabric_name);
					/* Update fabric name with MFW */
					qed_ops->update_fcoe_fabric_name(
					    qedf->cdev, (u8 *)&fabric_name);
				}
			} else
				QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_DISC,
				    "flp is NULL.\n");
	...
	struct qed_link_output *link)
{
	...
	case 10000:
		...
		break;
	/* Only newer distros have the 20/25/40/50/100 GB definitions */
	case 25000:
#ifdef FC_PORTSPEED_25GBIT
		lport->link_speed = FC_PORTSPEED_25GBIT;
#endif
		break;
	case 40000:
#ifdef FC_PORTSPEED_40GBIT
		lport->link_speed = FC_PORTSPEED_40GBIT;
#endif
		break;
	case 50000:
#ifdef FC_PORTSPEED_50GBIT
		lport->link_speed = FC_PORTSPEED_50GBIT;
#endif
		break;
	case 100000:
#ifdef FC_PORTSPEED_100GBIT
		lport->link_speed = FC_PORTSPEED_100GBIT;
#endif
		break;
	case 20000:
#ifdef FC_PORTSPEED_20GBIT
...
u8 validate_fcoe_app_tlv(struct qedf_ctx *qedf, struct qed_dcbx_get *get)
{
	u8 i;
	u8 fcoe_app_prio = 255;
	qedf->vprio = QEDF_INVALID;

	if (get->operational.enabled && get->operational.valid &&
	    get->operational.params.app_valid) {
		qedf->dcbx_session = QEDF_ENABLED;
		for (i = 0; i < get->operational.params.num_app_entries; i++) {
			if (get->operational.params.app_entry[i].proto_id ==
			    ETH_P_FCOE && get->operational.app_prio.fcoe >= 0 &&
				get->operational.app_prio.fcoe <= 7) {
				fcoe_app_prio = get->operational.app_prio.fcoe;
				qedf->vprio = QEDF_VALID;
				if (get->operational.params.pfc.prio[fcoe_app_prio])
					qedf->pfc = QEDF_ENABLED;
				break;
			}
		}
	} else {
			qedf->dcbx_session = QEDF_DISABLED;
	}
	return fcoe_app_prio;
}
...
static void qedf_dcbx_handler(void *dev, struct qed_dcbx_get *get, u32 mib_type)
{
	...
	fcoe_app_prio = validate_fcoe_app_tlv(qedf, get);

	switch (atomic_read(&qedf->dcbx)) {
	case QEDF_DCBX_PENDING:
		if (get->operational.enabled && get->operational.valid) {
			if (qedf_enforce_lossless_eth) {
				if (fcoe_app_prio != 255 &&
						get->operational.params
						.pfc.prio[fcoe_app_prio]) {
					QEDF_INFO(&qedf->dbg_ctx,
						QEDF_LOG_DISC,
						"PFC for FCoE %d (%d)",
						get->operational.params.pfc.prio
						[get->operational.app_prio.fcoe],
						get->operational.app_prio.fcoe);
					qedf_default_prio = -1;
				} else {
			 ...
			 * 3. Use the priorit y we get from the DCBX app tag
			 * Note: When enforcing lossless ethernet,
			 * we only perform step 3.
			 */
			if (qedf_default_prio > -1) {
				if (qedf_default_prio > 7) {
					...
					QEDF_INFO(&qedf->dbg_ctx,
						QEDF_LOG_DISC,
						"Forcing default prio %d, module parameter %d, app prio %d\n",
						qedf->prio, qedf_default_prio,
						fcoe_app_prio);
				} else {
					qedf->prio = qedf_default_prio;
					QEDF_INFO(&qedf->dbg_ctx,
						QEDF_LOG_DISC,
						"Using FCoE prio %d(module parameter), app prio %d\n",
						qedf->prio, fcoe_app_prio);
				}
			} else if (fcoe_app_prio == 255) {
				qedf->prio = QEDF_DEFAULT_PRIO;
				QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_DISC,
						"Forcing default FCoE prio %d because of invalid app prio %d\n",
						qedf->prio,
						get->operational.app_prio.fcoe);
			} else {
				qedf->prio = fcoe_app_prio;
			...
			qedf_dscp_pfc_enabled(qedf);
			qedf_dscp_pfc_get(qedf);
			qedf_dcbx_get_pfc(qedf);
			qedf_dcbx_get_pri_to_tc(qedf);
			qedf_dcbx_get_tc_bw(qedf);
			qedf_dcbx_get_tc_tsa(qedf);
			QEDF_INFO(&qedf->dbg_ctx,
					QEDF_LOG_INFO, "Mode = 0x%x.\n",
					qedf_dcbx_get_mode(qedf));
	...
	case QEDF_DCBX_DONE:
		if (!get->operational.enabled || !get->operational.valid) {
			QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_DISC,
					"DCBx session lost, transport may be lossy\n");
			qedf->dcbx_session = QEDF_DISABLED;
		} else if (fcoe_app_prio == 255) {
			QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_DISC,
					"DCBx - FCoE priority invalid, transport may be lossy\n");
			qedf->vprio = QEDF_VALID;
		} else if (!get->operational.params.pfc.prio[fcoe_app_prio]) {
			QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_DISC,
					"DCBx - PFC for FCoE %d (%d), transport may be lossy\n",
					get->operational.params.pfc.prio
					[get->operational.app_prio.fcoe],
					get->operational.app_prio.fcoe);
			qedf->pfc = QEDF_DISABLED;
		} else {
			QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_DISC,
		...
		if (qedf_enforce_lossless_eth) {
			QEDF_ERR(&qedf->dbg_ctx,
					"Tearing down sessions due to DCBx session disruption\n");
			atomic_set(&qedf->dcbx, QEDF_DCBX_PENDING);
			schedule_delayed_work(&qedf->stag_work, 0);
		}
		break;
	}
}
...
static struct qed_fcoe_cb_ops qedf_cb_ops = {
	{
		...
		.hw_attr_update = qedf_hw_attr_update,
	}
};
...
static int qedf_eh_abort(struct scsi_cmnd *sc_cmd)
{
	...
	if (!rval || io_req->sc_cmd != sc_cmd) {
		QEDF_ERR(&(qedf->dbg_ctx), "Freed/Incorrect io_req, io_req->sc_cmd=%px,"
			" sc_cmd=%px, port_id=%06x, bailing out.\n",
...
#endif
		QEDF_ERR(&(qedf->dbg_ctx), "rport not ready, io_req=%px, xid=0x%x sc_cmd=%px "
			"op=0x%02x, refcount=%d, port_id=%06x\n", io_req, io_req->xid,
			sc_cmd, sc_cmd->cmnd[0], refcount, rdata->ids.port_id);
		goto drop_rdata_kref;
	}

	rc = fc_block_scsi_eh(sc_cmd);
#if (defined(SLES_DISTRO) && (SLES_DISTRO > 0x1101 && SLES_DISTRO < 0x1200))
	if (rc != SUCCESS)
#else
	if (rc)
#endif /*SLES_DISTRO > 0x1101 */
		goto drop_rdata_kref;

	if (test_bit(QEDF_RPORT_UPLOADING_CONNECTION, &fcport->flags)) {
		QEDF_ERR(&(qedf->dbg_ctx), "Connection uploading, xid=0x%x., port_id=%06x\n",
			       io_req->xid, rdata->ids.port_id);
		while ((io_req->sc_cmd != NULL) && (wait_count != 0)) {
			msleep(100);
			wait_count--;
		}
		if (wait_count) {
	...
	QEDF_ERR(&(qedf->dbg_ctx), "Aborting io_req=%px sc_cmd=%px xid=0x%x "
		"fp_idx=%d, port_id=%06x.\n", io_req, sc_cmd, io_req->xid,
		io_req->fp_idx, rdata->ids.port_id);
...
static int qedf_eh_target_reset(struct scsi_cmnd *sc_cmd)
{
	...
	QEDF_ERR(NULL, "%d:0:%d:%u: TARGET RESET Issued...",
#else
...
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,2,0)
	QEDF_ERR(NULL, "%d:0:%d:%u: LUN RESET Issued...",
#else
...
{
	uint32_t wait_cnt = 120;
	struct qedf_rport *fcport = NULL;

	while (wait_cnt--) {
		if (atomic_read(&qedf->num_offloads))
			QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_DISC,
			    "Waiting for all uploads to complete num_offloads "
...
void qedf_ctx_soft_reset(struct fc_lport *lport)
{
	...
	if (qedf_wait_for_upload(qedf) == false) {
		QEDF_ERR(&qedf->dbg_ctx, "Cound not upload all sessions.\n");
		if (qedf_crash_on_err)
			BUG_ON(atomic_read(&qedf->num_offloads));
		else
	...
	if (test_bit(QEDF_IN_RECOVERY, &qedf->flags)) {
		QEDF_ERR(&qedf->dbg_ctx, "Already is in recovery,"
			"hence not calling link_update.\n");
		return;
	}
	...
	if (atomic_read(&qedf->dcbx) == QEDF_DCBX_DONE) {
		...
		QEDF_ERR(&qedf->dbg_ctx, "Got link update,"
                        "but DCBx is not done yet.\n");
	}
...
static int qedf_eh_host_reset(struct scsi_cmnd *sc_cmd)
{
	...
	if (atomic_read(&qedf->link_state) == QEDF_LINK_DOWN ||
		...
		QEDF_ERR(&(qedf->dbg_ctx),
		    "link_state %d flags=0x%lx.\n",
		    atomic_read(&qedf->link_state), qedf->flags);
		return FAILED;
	}
...
static int qedf_slave_configure(struct scsi_device *sdev)
{
	if (qedf_queue_depth) {
		...
		/* scsi_*_tcq were removed in 3.19 */
	...
	struct qedf_rport *fcport)
{
	...
	conn_info.rx_max_fc_pay_len = MAX_DATA_FIELD_SIZE;
...
#if defined FDMI_VERSION

	snprintf(fc_host_vendor_identifier(lport->host),
	    FC_SYMBOLIC_NAME_SIZE, "%s", "Marvell");
...
static void qedf_npiv_create_from_nvram(struct qedf_ctx *qedf)
{
	int rc;
	uint32_t i = 0;
	struct qed_fc_npiv_tbl *npiv_tbl;
	struct fc_vport_identifiers vpid;
	u64 wwnn = 0;
	char wwpn_str[32];
	char wwnn_str[32];

	npiv_tbl = kzalloc(sizeof(struct qed_fc_npiv_tbl), GFP_KERNEL);
	if (!npiv_tbl) {
		QEDF_ERR(&(qedf->dbg_ctx), "Could not allocate npiv_tbl.\n");
		return;
	}

	rc = qed_ops->get_fc_npiv(qedf->cdev, npiv_tbl);
	if (rc) {
		QEDF_ERR(&(qedf->dbg_ctx), "get_fc_npiv() call failed.\n");
		goto free;
	}

	if (npiv_tbl->num_wwpn < 1
	    || npiv_tbl->num_wwpn > QED_MAX_NPIV_ENTRIES) {
		QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_NPIV,
		    "Number of entries out of range: %d.\n",
		    npiv_tbl->num_wwpn);
		goto free;
	}

	QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_NPIV,
	    "Number of NPIV table entries: %d.\n", npiv_tbl->num_wwpn);

	/* Loop through table and precreate NPIV ports */
	for (i = 0; i < npiv_tbl->num_wwpn; i++) {
	...
	*shost)
{
	...
	eth_stats = kmalloc(sizeof(struct qed_eth_stats), GFP_KERNEL);
	if (!eth_stats) {
		QEDF_ERR(&(qedf->dbg_ctx), "Could not allocate memory for "
		    "qed_eth_stats.\n");
		goto free_fcoe_stats;
	}
	...
	/* Query firmware for eth stats */
	qed_ops->common->get_vport_stats(qedf->cdev,eth_stats);
	...
	qedf_stats->loss_of_signal_count =
		eth_stats->common.link_change_count;
...
static bool qedf_process_completions(struct qedf_fastpath *fp)
{
	...
	while (new_cqes) {
		...
		io_work = kzalloc(sizeof(struct qedf_io_work), GFP_ATOMIC);
		if (!io_work) {
...
static void qedf_sync_free_irqs(struct qedf_ctx *qedf)
{
	...
	if (qedf->int_info.msix_cnt) {
		for (i = 0; i < qedf->int_info.used_cnt; i++) {
			...
			synchronize_irq(qedf->int_info.msix[vector_idx].vector);
			irq_set_affinity_hint(
			    qedf->int_info.msix[vector_idx].vector,
			    NULL);
			irq_set_affinity_notifier(
			    qedf->int_info.msix[vector_idx].vector,
			    NULL);
			free_irq(qedf->int_info.msix[vector_idx].vector,
			    &qedf->fp_array[i]);
		}
	} else
...
static int qedf_request_msix_irq(struct qedf_ctx *qedf)
{
	...
	for (i = 0; i < qedf->num_queues; i++) {
		...
		rc = request_irq(qedf->int_info.msix[vector_idx].vector,
...
static int qedf_setup_int(struct qedf_ctx *qedf)
{
	 ...
	 */
	rc = qed_ops->common->set_fp_int(qedf->cdev, qedf->num_queues);
	if (rc <= 0) {
		QEDF_ERR(&(qedf->dbg_ctx), "set_fp_int falied rc=0x%x.\n",
		    rc);
		return 0;
	}
	...
	if (rc) {
		QEDF_ERR(&(qedf->dbg_ctx), "get_fp_int falied rc=0x%x.\n",
		    rc);
		return 0;
	}
...

{
...
#ifdef MODERN_VLAN
	/* Drop frames not on the FCoE VLAN */
        if (!__vlan_hwaccel_get_tag(skb, &vlan)) {
		if (vlan != qedf->vlan_id) {
			QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_LL2,
			    "Dropping FCoE frame due to VLAN mismatch: %04x %04x.\n",
			    qedf->vlan_id, vlan);
			kfree_skb(skb);
			return;
		}
		...
		QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_LL2,
		    "Dropping frame due to VLAN tag not being present.\n");
		kfree_skb(skb);
		return;
	}
#else
	if (vlan != qedf->vlan_id) {
		QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_LL2,
		    "Dropping FIP frame due to VLAN mismatch: %04x %04x.\n",
		    qedf->vlan_id, vlan);
		kfree_skb(skb);
		return;
	}
	...
	if (qedf->ctlr.state) {
		...
		if (compare_ether_addr(mac, qedf->ctlr.dest_addr)) {
#else
...
#else
		qedf_fip_recv(qedf, skb, skb_work->vlan);
#endif
		goto out;
...
#else
		qedf_recv_frame(qedf, skb, skb_work->vlan);
#endif
		goto out;
	} else
		goto err_out;
...
#endif
{
	...
	skb_work->qedf = qedf;
#ifndef MODERN_VLAN
	/* Save the VLAN argument we get for later examination */
	skb_work->vlan = vlan;
#endif
...
#ifdef ERROR_INJECT
	if (qedf->error_detect &&
	    comp_type == FCOE_GOOD_COMPLETION_CQE_TYPE) {
		qedf->error_detect--;
		comp_type = FCOE_ERROR_DETECTION_CQE_TYPE;
	}
#endif
	spin_lock_irqsave(&fcport->rport_lock, flags);
...
static int qedf_alloc_bdq(struct qedf_ctx *qedf)
{
	 ...
	 * individual buffers.
	 */
	qedf->bdq_pbl_list_num_entries = qedf->bdq_pbl_mem_size / QEDF_PAGE_SIZE;
...
static int qedf_alloc_global_queues(struct qedf_ctx *qedf)
{
	...
	for (i = 0; i < qedf->num_queues; i++) {
		...
		if (!qedf->global_queues[i]) {
			QEDF_ERR(&(qedf->dbg_ctx), "Unable to allocation "
				   "global queue %d.\n", i);
			goto mem_alloc_failure;
		}
		...
		if (!qedf->global_queues[i]->cq) {
			QEDF_ERR(&(qedf->dbg_ctx), "Could not allocate "
				   "cq.\n");
			status = -ENOMEM;
			goto mem_alloc_failure;
		}
...
static int qedf_set_fcoe_pf_param(struct qedf_ctx *qedf)
{
	...
	/* num_qeueus: Number of global queues (CQ/RQ). This should
	 * be <= number of available MSIX vector for the PF
	 */
...
static struct pci_error_handlers qedf_err_handler = {
	.error_detected = qedf_pci_error_detected,
	.slot_reset = qedf_pci_slot_reset,
	.resume = qedf_pci_resume,
};
...
retry_probe:
	...
	if (mode != QEDF_MODE_RECOVERY) {
		...
		if (qedf_blk_mq_can_queue)
			host->can_queue = qedf_blk_mq_can_queue;
		else
			host->can_queue = FCOE_PARAMS_NUM_TASKS /
			   qedf->num_queues;
		host->nr_hw_queues = qedf->num_queues;
		QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_INFO,
		  "Host is using shost blk-mq. can_queue=%d nr_hw_queues=%d\n",
		   host->can_queue, host->nr_hw_queues);

#elif defined(NR_HW_QUEUES) && defined(USE_BLK_MQ)
		host->use_blk_mq = qedf_use_blk_mq;
		if (shost_use_blk_mq(host)) {
			/*
			 * In blk-mq mode, set can_queue to the number of
			 * commands supported per hardware queue.
			 */
			if (qedf_blk_mq_can_queue)
				host->can_queue = qedf_blk_mq_can_queue;
			else
			...
			host->nr_hw_queues = qedf->num_queues;
			QEDF_INFO(&qedf->dbg_ctx, QEDF_LOG_INFO,
			  "Host is using blk-mq. can_queue=%d nr_hw_queues=%d\n",
			   host->can_queue, host->nr_hw_queues);
		} else {
			host->can_queue = FCOE_PARAMS_NUM_TASKS;
			QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_DISC,
			    "Host is not using blk-mq.\n");
		}
#else
		host->can_queue = FCOE_PARAMS_NUM_TASKS;
#endif
	...
	INIT_DELAYED_WORK(&qedf->board_disable_work, qedf_board_disable_work);
	...
	/* Create any NPIV ports stored in NVRAM */
	qedf_npiv_create_from_nvram(qedf);
...
/*
 * Hardware error handler code
 */
void qedf_board_disable_work(struct work_struct *work)
{
	struct qedf_ctx *qedf =
	    container_of(work, struct qedf_ctx, board_disable_work.work);

	QEDF_ERR(&(qedf->dbg_ctx), "Disabling PCI function.\n");
	__qedf_remove(qedf->pdev, QEDF_MODE_NORMAL);
}

static pci_ers_result_t
qedf_pci_error_detected(struct pci_dev *pdev, pci_channel_state_t state)
{
	struct qedf_ctx *qedf;
	...
	if (test_and_set_bit(QEDF_IN_RECOVERY, &qedf->flags)) {
		QEDF_ERR(&(qedf->dbg_ctx), "Already in recovery, not "
		    "scheduling slot reset.\n");
...
static pci_ers_result_t
qedf_pci_slot_reset(struct pci_dev *pdev)
{
	int rc;
	struct qedf_ctx *qedf;
	...
	if (rc) {
		QEDF_ERR(&qedf->dbg_ctx, "Failed to enable PCI device.\n");
		return PCI_ERS_RESULT_DISCONNECT;
	}

	QEDF_ERR(&qedf->dbg_ctx, "PCI Slot Reset.\n");

	/*
	 * Call common_ops->recovery_prolog to allow the MFW to quiesce
	 * any PCI transactions.
	 */
	qed_ops->common->set_recov_in_prog(qedf->cdev, true);
	qed_ops->common->recovery_prolog(qedf->cdev);
	...
	__qedf_remove(qedf->pdev, QEDF_MODE_RECOVERY);
	qed_ops->common->set_recov_in_prog(qedf->cdev, false);

	/* Perform cleanup of the PCIe registers */
#if defined CLEAR_NONFATAL_STATUS
	if (pci_aer_clear_nonfatal_status(pdev))
		QEDF_ERR(&qedf->dbg_ctx, "pci_aer_clear_nonfatal_status failed\n");
	else
		QEDF_ERR(&qedf->dbg_ctx, "pci_aer_clear_nonfatal_status succeeded\n");
#else
	if (pci_cleanup_aer_uncorrect_error_status(pdev))
		QEDF_ERR(&qedf->dbg_ctx, "pci_cleanup_aer_uncorrect_error_status failed\n");
	else
		QEDF_ERR(&qedf->dbg_ctx, "pci_cleanup_aer_uncorrect_error_status succeeded\n");
#endif
...
static void
qedf_pci_resume(struct pci_dev *pdev)
{
	struct qedf_ctx *qedf;
	...
	QEDF_ERR(&qedf->dbg_ctx, "PCI Resume.\n");
	__qedf_probe(qedf->pdev, QEDF_MODE_RECOVERY);
	clear_bit(QEDF_IN_RECOVERY, &qedf->flags);
...
void qedf_schedule_hw_err_handler(void *dev, enum qed_hw_err_type err_type)
{
	...
	QEDF_ERR(&(qedf->dbg_ctx), "Hardware error handler scheduled, "
	    "event=%d.\n", err_type);

	if (test_bit(QEDF_IN_RECOVERY, &qedf->flags)) {
		QEDF_ERR(&(qedf->dbg_ctx), "Already in recovery, not "
		    "scheduling board disable work.\n");
		return;
	}
...
void qedf_get_protocol_tlv_data(void *dev, void *data)
{
	...
	fcoe->fcoe_rxq_depth = fcoe->fcoe_txq_depth = FCOE_PARAMS_NUM_TASKS;
...
void qedf_stag_change_work(struct work_struct *work)
{
	...
	if (test_bit(QEDF_IN_RECOVERY, &qedf->flags)) {
		QEDF_ERR(&qedf->dbg_ctx, "Already is in recovery,"
			"hence not calling software context reset.\n");
		return;
	}
...
/*
 * Hardware update attribute handler
 */
void qedf_hw_attr_update(void *dev, enum qed_hw_info_change attr)
{
	struct qedf_ctx *qedf = dev;

	/* Sanity check cookie we're passed in */
	if (!qedf) {
		QEDF_INFO(NULL, QEDF_LOG_EVT,
		    "qedf is NULL so ignoring hw_attr_update event.\n");
		return;
	}

	switch (attr) {
	case QED_HW_INFO_CHANGE_OVLAN:
		/* Don't do the reset if we're unloading or quiesced */
		if (atomic_read(&qedf->link_state) == QEDF_LINK_DOWN ||
		    test_bit(QEDF_UNLOADING, &qedf->flags) ||
		    test_bit(QEDF_DBG_STOP_IO, &qedf->flags))
			return;
		QEDF_ERR(&(qedf->dbg_ctx),
		    "STAG changed, queuing stag reset work.\n");
		schedule_delayed_work(&qedf->stag_work, 0);
		break;
	default:
		QEDF_INFO(&(qedf->dbg_ctx), QEDF_LOG_DISC,
 ...
 */
int qedf_get_fcoe_capabilities(void *dev, struct qed_fcoe_caps *p_caps)
{
	struct qedf_ctx *qedf;

	if (dev == NULL) {
		QEDF_INFO(NULL, QEDF_LOG_EVT,
		    "dev is NULL so ignoring get_fcoe_capabilities request.\n");
		return -EINVAL;
	}
	qedf = (struct qedf_ctx *)dev;

	p_caps->max_ios = NUM_RW_TASKS_PER_CONNECTION;
	p_caps->max_log = QEDF_MAX_SESSIONS;
	p_caps->max_exch = FCOE_PARAMS_NUM_TASKS;
	p_caps->max_npiv = QEDF_MAX_NPIV;
	p_caps->max_tgt = QEDF_MAX_SESSIONS;
	p_caps->max_outstnd = FCOE_PARAMS_NUM_TASKS;

	/* Zero considered good status */
	return 0;
}
...
static int __init qedf_init(void)
{
	...
	/* Initialize core driver operations */
	qed_ver = qed_get_protocol_version(QED_PROTOCOL_FCOE);
	if (qed_ver !=  QEDF_FCOE_INTERFACE_VERSION) {
		QEDF_ERR(NULL, "Version mismatch [%08d != %08d]\n",
			  qed_ver, QEDF_FCOE_INTERFACE_VERSION);
		goto err1;
	}

	qed_ops = qed_get_fcoe_ops(QEDF_FCOE_INTERFACE_VERSION);
	if (!qed_ops) {
	...
	/* Assign callbacks to qed_ops struct */
	qedf_cb_ops.get_login_failures = qedf_get_login_failures;
	qedf_cb_ops.get_fcoe_capabilities = qedf_get_fcoe_capabilities;
